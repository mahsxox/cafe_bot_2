...
async def menu(message: Message):
    await message.reply(text='Введите название блюа, которое желаете найти:')


class SearchFilter(BaseFilter):
    async def __call__(self, message: Message) -> bool:
        return message.text.title() in products


# Хендлер для поиска по ключам
@router.message(SearchFilter())
async def product_search_handler(message: Message):
    product_name = message.text
    product_price = products[product_name]
    response = (
        f"Название: {product_name}\n"
        f"Цена: {product_price} руб.\n\n"
        f"Хотите добавить это блюдо в корзину?"
    )

    await message.answer(response, reply_markup=await kb.search_keyboard(product_name))
